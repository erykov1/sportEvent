...
class ReportSpec extends Specification implements ReportSample {
  ...
  def "Should create report"() {
    when: "creates report"
      def result = reportFacade.createReport(new CreateReportDto("jane", "doe", HANDBALL_EVENT))
    then: "report is created"
      equalsReport(result, createReport(reportId: result.reportId, name: "jane", surname: "doe", reportStatus: ReportStatusDto.PENDING,
  ...
  def "Should not create report if try to report to not existing sport event"() {
    when: "creates report to not existing sport event"
      reportFacade.createReport(new CreateReportDto("jane", "doe", new Random().nextLong()))
  ...
  def "Should create report for the same user if try to assign to two sport events that are in different time"() {
    given: "user assign to first event"
      def handballEvent = reportFacade.createReport(new CreateReportDto("jane", "doe", HANDBALL_EVENT))
    when: "user assigns to other event"
      def footballEvent = reportFacade.createReport(new CreateReportDto("jane", "doe", FOOTBALL_EVENT))
    then: "user is assigned to two sport events"
      equalsReports(reportFacade.getAllUserReports("jane123"), [createReport(reportId: handballEvent.reportId, name: "jane", surname: "doe", reportStatus: ReportStatusDto.PENDING,
          reportedAt: footballEvent.reportedAt, eventId: HANDBALL_EVENT
      ), createReport(reportId: handballEvent.reportId, name: "jane", surname: "doe", reportStatus: ReportStatusDto.PENDING,
  ...
  def "Should not create report for the sport event if user is already reported to other sport event"() {
    given: "user report for the first event"
      def report = reportFacade.createReport(new CreateReportDto("jane", "doe", HANDBALL_EVENT))
    and: "gets accepted"
      reportFacade.acceptReport(report.reportId)
    when: "user reports for other sport event that is in the same time"
      reportFacade.createReport(new CreateReportDto("jane", "doe", HANDBALL_EVENT))
  ...
  def "Should get all reports with pending status"() {
    given: "there are some reports with pending status"
      def handballEvent = reportFacade.createReport(new CreateReportDto("jane", "doe", HANDBALL_EVENT))
      def footballEvent = reportFacade.createReport(new CreateReportDto("mike", "jones", FOOTBALL_EVENT))
    ...
    equalsReports(result, [createReport(reportId: handballEvent.reportId, name: "jane", surname: "doe", reportStatus: ReportStatusDto.PENDING,
        reportedAt: footballEvent.reportedAt, eventId: HANDBALL_EVENT
    ), createReport(reportId: handballEvent.reportId, name: "mike", surname: "jones", reportStatus: ReportStatusDto.PENDING,
        reportedAt: footballEvent.reportedAt, eventId: FOOTBALL_EVENT)])
  }
  ...
  def "Should get all reports with declined status"() {
    ...
    then: "gets all pending status"
    equalsReports(result, [createReport(reportId: handballEvent.reportId, name: "jane", surname: "doe", reportStatus: ReportStatusDto.DECLINED,
        reportedAt: footballEvent.reportedAt, eventId: HANDBALL_EVENT
    ), createReport(reportId: handballEvent.reportId, name: "mike", surname: "jones", reportStatus: ReportStatusDto.DECLINED,
        reportedAt: footballEvent.reportedAt, eventId: FOOTBALL_EVENT)])
  }
}
